...
Public Class ThickenerGenerator
    ...
    Private Sub FillGeometryData()
        ...
        With ObjThickGen
            GD.Origin.x = Val(.txtxOrigin.Text)
            GD.Origin.y = Val(.txtyOrigin.Text)
            GD.Origin.z = Val(.txtzOrigin.Text)
            GD.Alpha = Deg2Rad(Val(.txtAlpha.Text)) ' Converted to Radians
            GD.Beta = Deg2Rad(Val(.txtBeta.Text)) ' Converted to Radians
            GD.sDelta = Deg2Rad(Val(.txtsDelta.Text)) ' Converted to Radians
            GD.nRB = CLng(Val(.txtnRB.Text))
            GD.nColRB = CLng(Val(.txtnColRB.Text))
            GD.idBP = Val(.txtidBP.Text)
            GD.odBP = Val(.txtodBP.Text)
            GD.pcdBolts = Val(.txtpcdBolts.Text)
            GD.idCP = Val(.txtidCP.Text)
            GD.odCP = Val(.txtodCP.Text)
            GD.dCC = Val(.txtdCC.Text)
            GD.hCC = Val(.txthCC.Text)
            GD.hCCTop = Val(.txthCCTop.Text)
            GD.dUF = Val(.txtdUF.Text)
            GD.hUF = Val(.txthUF.Text)
            GD.dCR = Val(.txtdCR.Text)
            GD.bCR = Val(.txtbCR.Text)
            GD.hCR = Val(.txthCR.Text)
            GD.hDroop = Val(.txthDroop.Text)
            '/--------- Modified -------------/
            If GD.hDroop <> 0.0 Then
                GD.rDroop = Val(.txtrDroop.Text)
            End If
            '/--------- Modified -------------/
            GD.nBoltsCC = CLng(Val(.txtnBoltsCC.Text))
            GD.LHalfChord = Val(.txtHalfChordLength.Text)
            '/--------- Modified -------------/
            If GD.hDroop <> 0.0 Then
                GD.Theta = Deg2Rad(Val(.txtTheta.Text)) ' Converted to Radians
            End If
            '/--------- Modified -------------/
            GD.dTank = Val(.txtdTank.Text)
            GD.hWall = Val(.txthWall.Text)
            GD.FreeBoard = Val(.txtFreeBoard.Text)
            GD.Omega = Deg2Rad(Val(.txtOmega.Text))
            GD.bLaun = Val(.txtbLaun.Text)
            GD.hLaun = Val(.txthLaun.Text)
            GD.dLaun = (GD.dTank - 2 * GD.bLaun)
            GD.MaxPlateDim = Val(.txtMaxPlateDim.Text)
            GD.AspectRatio = Val(.txtAspectRatio.Text)
    ...
    Private Function CalculateStartNode(Div As Long, iDelta As Double, ByRef Mesh As MeshData, ShellComp As Component) As NodeCoOrds

        Dim startNode As NodeCoOrds

        Select Case ShellComp
            Case Component.LFloor
                startNode.x = GD.Origin.x + (GD.dTank / 2 - GD.bLaun + Div * Mesh.Hp) * Math.Cos(iDelta)
                startNode.y = BP.sNodeLF.y
                startNode.z = GD.Origin.z + (GD.dTank / 2 - GD.bLaun + Div * Mesh.Hp) * Math.Sin(iDelta)
            Case Component.WLWall
                startNode.x = GD.Origin.x + (GD.dTank / 2) * Math.Cos(iDelta)
                startNode.y = BP.sNodeTWL.y + Div * Mesh.Hp
                startNode.z = GD.Origin.z + (GD.dTank / 2) * Math.Sin(iDelta)
            Case Component.WFreeBoard
                startNode.x = GD.Origin.x + (GD.dTank / 2) * Math.Cos(iDelta)
                startNode.y = BP.sNodeTWFB.y + Div * Mesh.Hp
                startNode.z = GD.Origin.z + (GD.dTank / 2) * Math.Sin(iDelta)
            Case Component.Wall
                startNode.x = GD.Origin.x + (GD.dTank / 2) * Math.Cos(iDelta)
                startNode.y = BP.sNodeTWL.y - Div * Mesh.Hp
    ...
    Private Function CalculateIntermediateNode(Div As Long, ByRef jdDelta As Double, ByRef Mesh As MeshData, ShellComp As Component) As NodeCoOrds

        Dim intermediateNode As NodeCoOrds

        Select Case ShellComp
            Case Component.LFloor
                intermediateNode.x = (GD.dTank / 2 - GD.bLaun + Div * Mesh.Hp) * Math.Cos(jdDelta)
                intermediateNode.y = BP.sNodeLW.y
                intermediateNode.z = (GD.dTank / 2 - GD.bLaun + Div * Mesh.Hp) * Math.Sin(jdDelta)
            Case Component.WLWall
                intermediateNode.x = (GD.dTank / 2) * Math.Cos(jdDelta)
                intermediateNode.y = BP.sNodeTWL.y + Div * Mesh.Hp
                intermediateNode.z = (GD.dTank / 2) * Math.Sin(jdDelta)
            Case Component.WFreeBoard
                intermediateNode.x = (GD.dTank / 2) * Math.Cos(jdDelta)
                intermediateNode.y = BP.sNodeTWFB.y + Div * Mesh.Hp
                intermediateNode.z = (GD.dTank / 2) * Math.Sin(jdDelta)
            Case Component.Wall
                intermediateNode.x = (GD.dTank / 2) * Math.Cos(jdDelta)
                intermediateNode.y = BP.sNodeTWL.y - Div * Mesh.Hp
    ...
    Public Sub GenerateMesh(ByRef Mesh As MeshData, ByRef IFMesh As MeshData, ByRef pGr() As Integer, ByRef NodesGr() As NodeCoOrdsGroup, ByRef IFNodesGr() As NodeCoOrdsGroup,
                            GrName As String, ByRef SeqNodeNum As Long, ByRef SeqPlateNum As Long, Comp As Component)
        ...
        For Div = 1 To CInt(Mesh.Nd)
            ...
            If TrType = TransType.Inc Then
                If Div > (Mesh.Nd - Math.Abs(Trans)) Then
                    UpperLimit += (Div - (Mesh.Nd - Math.Abs(Trans)))
                Else
            ...
            For I = 0 To GD.nRB - 1
                ...
                sNode = CalculateStartNode(Div, iDelta, Mesh, Comp)
                ...
                For J = 1 To UpperLimit - 1
                    ...
                    iNode = CalculateIntermediateNode(Div, jdDelta, Mesh, Comp)
    ...
    Public Sub CalculateHeightAdjustment(ByRef IFMesh As MeshData, ByRef AdjHeight As Double)
        ...
        For I = 0 To 0
            ...
            With IFNodesGr(0).Nodes(UBound(IFNodesGr(0).Nodes))
                sNode = CalculateStartNode(Div, iDelta, IFMesh, Component.Wall)
            ...
            For J = 1 To 1
                jTheta = J * (GD.Omega / UpperLimit)
                jdDelta = iDelta + jTheta
                iNode = CalculateIntermediateNode(Div, jdDelta, IFMesh, Component.Wall)
                jXChord = iNode.x : jYChord = iNode.y : jZChord = iNode.z
                With IFNodesGr(0).Nodes(UBound(IFNodesGr(0).Nodes))
    ...
    Public Sub GenerateInterfaceMesh(AdjHeight As Double, ByRef pGr() As Integer, ByRef NodesGr() As NodeCoOrdsGroup, ByRef IFNodesGr() As NodeCoOrdsGroup, ByVal GrName As String,
                                     ByRef SeqNodeNum As Long, ByRef SeqPlateNum As Long, Comp As Component)
    ...
    Public Sub GenerateBeams(ByRef NodesGr() As NodeCoOrdsGroup, ByRef Beams() As Beam, ByRef SeqPlateNum As Long)
    ...
    Private Sub GeneratePlates(ByRef NodesGr() As NodeCoOrdsGroup, ByRef PlateGroup() As Integer, ByRef SeqPlateNum As Long, Optional Com As Component = Component.None, Optional BPOmega As Double = vbEmpty)
    ...
    Private Sub GenerateNodes(iBDiv As Double, ByRef iDiv As Integer, ByRef D() As Double, ByRef NodesGr() As NodeCoOrdsGroup, ByRef ieNode() As NodeCoOrds, ByRef SeqNodeNum As Long,
                              ByRef Lres As Double, nSegs As Long, Com As Component, Optional BPOmega As Double = vbEmpty, Optional yDiff As Double = vbEmpty) 'Newly added optional parameter
...
ReduceDivWidth:
        If Com = Component.FloorPlate Then
            ...
            D(iDiv) = D(iDiv - 1) - (2 * iBDiv) * Math.Cos(GD.Alpha)
    ...
    Public Sub GenerateCompMesh(ByRef NodesGr() As NodeCoOrdsGroup, ByRef IFNodesGr() As NodeCoOrdsGroup, ByRef pGr() As Integer, ByRef SeqNodeNum As Long, ByRef SeqPlateNum As Long,
                                Com As Component, Optional OD As Double = vbEmpty, Optional ID As Double = vbEmpty, Optional Height As Double = vbEmpty)
    ...
    Public Sub GenerateVAStiffenerMesh(ByRef NodesGr() As NodeCoOrdsGroup, ByRef NodesCR() As NodeCoOrdsGroup, ByRef NodesVA() As NodeCoOrdsGroup, ByRef NodesUF() As NodeCoOrdsGroup,
                                       ByRef pGr() As Integer, ByRef SeqNodeNum As Long, ByRef SeqPlateNum As Long)
    ...
    Private Sub GenerateVASNodes(iBDiv As Double, iDiv As Integer, ByRef D() As Double, ByRef NodesGr() As NodeCoOrdsGroup, ByRef SeqNodeNum As Long, Com As Component, Optional yDiff As Double = vbEmpty)
    ...
    Private Sub GenerateVASPlates(ByRef NodesGr() As NodeCoOrdsGroup, ByRef PlateGroup() As Integer, ByRef SeqPlateNum As Long, Optional Com As Component = Component.None)
    ...
    Private Sub CreateNewPlate(ByRef SeqPlateNum As Long, ByRef PlateGr() As Integer, NodeA As Long, NodeB As Long, NodeC As Long, NodeD As Long)
    ...
    Private Sub CreateNewNode(ByRef SeqNodeNum As Long, ByRef NodesGroup() As NodeCoOrdsGroup, index As Integer, x As Double, y As Double, z As Double)
